...
contract L1BossBridgeTest is Test {
    ...
    function testCanTransferFromVaultToVault() public {
        ...
        // Can trigger the deposit event,  self transfer token to the vault

        console2.log("sss: ", IERC20(token).balanceOf(address(vault)));
        console2.log("aaa: ", IERC20(token).balanceOf(address(attacker)));
    ...
    function testSignatureReplay() public {
        ...
        // An attacker deposists tokens to L2
        vm.startPrank(attacker);
        token.approve(address(tokenBridge), type(uint256).max);
